...
class PacketCapture(object):
    ...
    def __init__(self, ixnObj=None, portMgmtObj=None):
        ...
        Example:
            ...
            pktCaptureObj.getCapFile(port=[ixChassisIp, '2', '1'],
                                    typeOfCapture='data', saveToTempLocation='c:\\Results',
    ...
    def packetCaptureConfigPortMode(
        self, port, portRxMode="capture", enableControlPlane=True, enableDataPlane=True
    ):
        """
        Description
        ...
        """
        if enableControlPlane is True:
            self.enableControlPlane = True
        if enableDataPlane is True:
        ...
        self.ixnObj.patch(
            self.ixnObj.httpHeader + vport + "/capture",
            data={
    ...
    def packetCaptureStop(self):
        ...
        self.ixnObj.waitForComplete(
            response,
            self.ixnObj.sessionUrl + "/operations/stopcapture/" + response.json()["id"],
        )
    ...
    def packetCaptureGetCurrentPackets(
        self, getUpToPacketNumber=20, capturePacketsToFile=True
    ):
        """
        Description
        ...
        if capturePacketsToFile:
            ...
            if self.enableDataPlane is False and self.enableControlPlane is False:
        ...
        for eachTypeOfCaptures, totalCapturedPackets in zip(
            ("data", "control"), (totalDataCapturedPackets, totalControlCapturedPackets)
        ):
            self.ixnObj.logInfo(
            ...
            for packetIndex in range(1, int(totalCapturedPackets)):
                ...
                response = self.ixnObj.get(
                    self.ixnObj.httpHeader + vport + "/capture/currentPacket/stack",
                    silentMode=False,
                )
                ...
                for eachStack in response.json():
                    ...
                    if capturePacketsToFile:
                        if eachTypeOfCaptures == "data":
                            with open(
                                packetCaptureFilenameData, "a"
                            ) as packetCaptureFile:
                                packetCaptureFile.write("\nStack: %s\n" % displayName)

                        if eachTypeOfCaptures == "control":
                            with open(
                                packetCaptureFilenameControl, "a"
                            ) as packetCaptureFile:
                                packetCaptureFile.write("\nStack: %s\n" % displayName)

                    response = self.ixnObj.get(
                        self.ixnObj.httpHeader + stackIdObject + "/field",
                        silentMode=False,
                    )
    ...
    def packetCaptureGetCurrentPacketsHex(self, getUpToPacketNumber=20):
        ...
        for eachTypeOfCaptures, totalCapturedPackets in zip(
            ...
            for packetIndex in range(1, int(totalCapturedPackets)):
                ...
                data = {"arg1": "-packetHex"}
                response = self.ixnObj.get(
                    self.ixnObj.httpHeader + vport + "/capture/currentPacket",
                    data=data,
                    silentMode=False,
                )
    ...
    def getCapFile(
        ...
        response = self.ixnObj.post(
            self.ixnObj.sessionUrl + "/operations/savecapturefiles", data=data
        )
        self.ixnObj.waitForComplete(
            response, self.ixnObj.httpHeader + response.json()["url"], timeout=300
        )
        ...
        if typeOfCapture == "control":
            if "\\" not in saveToTempLocation:
                # For Linux
                capFileToGet = ("/home/ixia_apps/web/platform/apps-resources/ixnetworkweb/"
                                + "configs/captures/{0}/{1}_SW.cap".format(saveToTempLocation, vportName))
            else:
                # For Windows
        ...
        if typeOfCapture == "data":
            if "\\" not in saveToTempLocation:
                # For Linux
                capFileToGet = ("/home/ixia_apps/web/platform/apps-resources/ixnetworkweb/" +
                                "configs/captures/{0}/{1}_HW.cap".format(saveToTempLocation, vportName))
            else:
                capFileToGet = capfilePathName
