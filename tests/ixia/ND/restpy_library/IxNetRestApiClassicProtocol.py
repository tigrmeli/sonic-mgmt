...
class ClassicProtocol(object):
    ...
    def getPortsByProtocol(self, protocolName):
        ...
        for vport in vportList:
            ...
            if response.json()["enabled"] is True:
                # 192.168.70.11:1:5
        ...
        for eachVport in vportList:
            for eachProtocol in protocolList:
                ...
                protocolResponse = self.ixnObj.get(
                    self.ixnObj.httpHeader + eachVport + "/protocols/" + eachProtocol
                )
                ...
                if response.json() != []:
                    if (
                        response.json()[0]["enabled"] is True
                        and protocolResponse.json()["runningState"] == "started"
                    ):
    ...
    def getProtocolSessionsStats(self, portName, protocol):
        ...
        if response.json() != []:
            ...
            raise IxNetRestApiException(
                "No {0} protocol configured on port {1}".format(protocol, portName)
            )
            ...
            if stats[session]["Port Name"] == portName:
                self.ixnObj.logInfo(
                    "\n\tPortName: {0}\n\t   TotalSessionsUp: {1}\n\t   TotalSessionsConfigured: {2}".format(
                        stats[session]["Port Name"], sessionsUp, totalSessions
                    ),
                    timestamp=False,
                )
        ...
        for eachRouterInstance in RouterInstanceList:
            ...
            for eachRouteRange in RouteRangeInstanceList:
                ...
                if RouteRangeNetwork == routeRange:
                    ...
                    return
            raise IxNetRestApiException(
                "Route range: {0} does not exist in protocol: {1} port: {2}".format(
                    routeRange, protocol, portName
                )
            )
    ...
    def createRouteRangeOnProtocol(self, portName, protocol, routeRange):
        ...
        Parameters
            ...
            eg: {'enabled': 'True', 'mask': 24, 'numberOfRoutes': 5, 'networkNumber': '8.7.7.1',
                'metric': 10, 'origin': 'externalType1'}
                {'enabled': 'True', 'maskWidth': 64, 'numberOfRoute': 10,
                    'firstRoute': '7::1', 'metric': 10, 'nextHop': '7::2'}

        Syntax
            POST: /api/v1/sessions/{id}/ixnetwork/vport/{id}/protocols/ospf/router/{id}/routeRange/{id}
            DATA: {'enabled': 'True', 'mask': 24, 'numberOfRoutes': 5,
                    'networkNumber': '7.7.7.1', 'metric': 10, 'origin': 'externalType1'}

            POST: /api/v1/sessions/{id}/ixnetwork/vport/{id}/protocols/ospfV3/router/{id}/routeRange/{id}
            DATA: {'enabled': 'True', 'mask': 64, 'numberOfRoutes': 5,
                'firstRoute': '7::1', 'metric': 10, 'type': 'anotherArea', 'addressFamily': 'unicast'}

            POST: /api/v1/sessions/{id}/ixnetwork/vport/{id}/protocols/eigrp/router/{id}/routeRange/{id}
            DATA: {'enabled': 'True', 'mask': 24, 'numberOfRoutes': 10,
                     'firstRoute': '7.7.7.1', 'metric': 10, 'nextHop': '7.7.7.2'}

            POST: /api/v1/sessions/{id}/ixnetwork/vport/{id}/protocols/rip/router/{id}/routeRange/{id}
            DATA: {'enabled': 'True', 'maskWidth': 24, 'noOfRoutes': 10,
                    'firstRoute': '7.7.7.1', 'metric': 10, 'nextHop': '7.7.7.2'}

            POST: /api/v1/sessions/{id}/ixnetwork/vport/{id}/protocols/ripng/router/{id}/routeRange/{id}
            DATA: {'enabled': 'True', 'maskWidth': 64, 'numberOfRoute': 10,
                    'firstRoute': '7::1', 'metric': 10, 'nextHop': '7::2'}
    ...
    def verifyProtocolSessionsUp(
        ...
        Parameters
            protocolViewName: <str>: The protocol view name. Get this name from API browser
                                                        or in IxNetwork GUI statistic tabs.
            ...
            for session in stats.keys():
                ...
                if totalExpectedSessionsUp != 0:
                    ...
                    if counter < timeout and sessionsUp != totalExpectedSessionsUp:
                        self.ixnObj.logInfo(
                            "\t   Protocol Session is still down", timestamp=False
                        )
