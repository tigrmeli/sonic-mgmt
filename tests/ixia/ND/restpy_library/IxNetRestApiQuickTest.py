...
class QuickTest(object):
    ...
    def applyQuickTest(self, qtHandle):
        ...
        waitForComplete = self.ixnObj.waitForComplete(
            response,
            self.ixnObj.sessionUrl+'/quickTest/operations/apply/'+response.json()['id']
        )
    ...
    def verifyQuickTestInitialization(self, quickTestHandle):
        ...
        for counter in range(1, applyQuickTestCounter + 1):
            ...
            if ixNetworkVersionNumber >= 8:
                ...
                if counter < applyQuickTestCounter and currentAction == 'TransmittingFrames':
                    self.ixnObj.logInfo('\nVerifyQuickTestInitialization is done applying ' +
                                        'configuration and has started transmitting frames\n')
                break
            ...
            if counter == applyQuickTestCounter:
                if ixNetworkVersionNumber >= 8 and currentAction != 'TransmittingFrames':
                    ...
                    if currentAction == 'ApplyFlowGroups':
                        self.ixnObj.logInfo(
                            '\nIxNetwork is stuck on Applying Flow Groups. ' +
                            'You need to go to the session to FORCE QUIT it.\n'
                        )
    ...
    def monitorQuickTestRunningProgress(self, quickTestHandle, getProgressInterval=10):
        ...
        while True:
            ...
            if isRunning is True:
                ...
                if bool(re.match('^Trial.*', currentRunningProgress)) is False:
                    if waitForRunningProgressCounter < 30:
                        self.ixnObj.logInfo(
                            'isRunning=True. Waiting for trial ' +
                            'runs {0}/30 seconds'.format(waitForRunningProgressCounter)
                        )
    ...
    def getQuickTestResultPath(self, quickTestHandle):
        ...
        response = self.ixnObj.get(self.ixnObj.httpHeader + quickTestHandle + '/results')
        # "resultPath":
        # "C:\\Users\\hgee\\AppData\\Local\\Ixia\\IxNetwork\\ -> next line is the continue
        # data\\result\\DP.Rfc2544Tput\\10694b39-6a8a-4e70-b1cd-52ec756910c3\\Run0001"
    ...
    def getQuickTestPdf(
        ...
        response = self.ixnObj.post(
            self.ixnObj.httpHeader+quickTestHandle+'/operations/generateReport', data={'arg1': quickTestHandle}
        )
        if response.json()['url'] != '':
            ...
            if where == 'localWindows':
                ...
                self.fileMgmtObj.copyFileWindowsToLocalWindows(
                    response.json()['result'], copyToLocalPath, renameDestinationFile, includeTimestamp
                )
            if where == 'remoteWindows':
                ...
                # self.ixnObj.copyFileWindowsToRemoteWindows(
                # response.json()['result'], copyToLocalPath, renameDestinationFile, includeTimestamp
                # )
                self.fileMgmtObj.copyFileWindowsToRemoteWindows(
                    resultPath, copyToLocalPath, renameDestinationFile, includeTimestamp
                )
            if where == 'remoteLinux':
                ...
                self.fileMgmtObj.copyFileWindowsToLocalLinux(
                    linuxResultPath+'\\TestReport.pdf', copyToLocalPath, renameDestinationFile, includeTimestamp
                )
        else:
    ...
    def configQuickTest(self, quickTestName, numOfTrials=1):
        ...
        response = self.ixnObj.post(
            url, data={"forceApplyQTConfig": "true", "mode": "existingMode", "name": quickTestName}
        )
