...
class Connect:
    ...
    def __init__(
        ...
        verifySslCert=False,
        includeDebugTraceback=True,
        sessionId=None,
        httpsSecured=None,
        apiKey=None,
        generateLogFile=True,
        robotFrameworkStdout=False,
        ...
        Notes
            ...
            If you still want to use http, you need to add -restInsecure
                    to the IxNetwork.exe appliaction under "target".
        ...
        if self.serverOs == "windowsConnectionMgr":
            if httpsSecured is False or self.apiServerPort is None:
                raise IxNetRestApiException(
                    "If using windowsConnectionMgr, you must state httpsSecured=True|False and a serverIpPort to use."
                )

        self.httpScheme = "http"
        # This will dynamically change to https.
        ...
        # This is the only api server that requires user to state
        #   httpsSecured=True|False because 8.40 users could be using http.
        ...
        if linuxChassisIp:
            self.connectToLinuxIxosChassis(
                self.linuxChassisIp, self.username, self.password
            )
            return
        ...
        if serverOs == "windowsConnectionMgr":
            ...
            if sessionId:
                url = "{0}://{1}:{2}/api/v1/sessions/{3}".format(
                    self.httpScheme, apiServerIp, self.apiServerPort, str(sessionId)
                )
                try:
                    response = self._session.request(
                        "GET", url, verify=self.verifySslCert, allow_redirects=False
                    )
                    ...
                    errMsg = (
                        "Connecting to existing config failed on a GET: {0}".format(
                            errMsg
                        )
                    )
                ...
                self.sessionId = "{0}://{1}:{2}/api/v1/sessions/{3}".format(
                    self.httpScheme, apiServerIp, self.apiServerPort, str(sessionId)
                )
                self.apiSessionId = "/api/v1/sessions/{0}/ixnetwork".format(
                    str(sessionId)
                )
        ...
        if serverOs == "linux":
            ...
            if self.manageSessionMode:
                if self.apiKey is None:
                    response = self.post(
                        "https://{}:{}/api/v1/auth/session".format(
                            self.linuxApiServerIp, self.apiServerPort
                        ),
                        data={"username": "admin", "password": "admin"},
                        headers={"content-type": "application/json"},
                    )
                ...
                self.sessionUrl = "https://{0}:{1}/ixnetworkweb/api/v1/sessions".format(
                    self.linuxApiServerIp, self.apiServerPort
                )
                return
            ...
            if sessionId:
                if self.apiKey is None:
                    url = "https://{0}:{1}/api/v1/auth/session".format(
                        self.linuxApiServerIp, self.apiServerPort
                    )
                    response = self.post(
                        url,
                ...
                self.sessionId = "https://{0}:{1}/api/v1/sessions/{2}".format(
                    self.linuxApiServerIp, self.apiServerPort, str(sessionId)
                )
                ...
                response = self.get(
                    "https://{0}:{1}/api/v1/sessions/{2}".format(
                        self.linuxApiServerIp, self.apiServerPort, str(sessionId)
                    )
                )

                if self.webQuickTest is False:
                    ...
                    matchHeader = re.match(
                        r"(https://[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+(:[0-9]+)?)",
                        self.sessionId,
                    )
                ...
                if self.webQuickTest:
                    self.sessionId = (
                        "https://{0}:{1}/ixnetwork/api/v1/sessions/{2}".format(
                            self.linuxApiServerIp, self.apiServerPort, str(sessionId)
                        )
                    )
            ...
            # Create new session: connectToLinuxApiServer API knows whether to create a new session or connect to
            #   an existing session by looking at the self.apiKey.
            if apiKey is None and sessionId is None:
                self.connectToLinuxApiServer(
                    self.linuxApiServerIp,
                    self.apiServerPort,
                    username=username,
                    password=password,
                    verifySslCert=verifySslCert,
                    timeout=self.linuxApiServerTimeout,
                )
        ...
        #    Delete the session when script is done if deleteSessionAfterTest = True.
    ...
    def get(
        ...
        while True:
            ...
            try:
                ...
                if silentMode is False:
                    for redirectStatus in response.history:
                        if "307" in str(response.history):
                            self.logInfo(
                                "\t{0}: {1}".format(redirectStatus, response.url),
                                timestamp=False,
                            )
                ...
                if not str(response.status_code).startswith("2"):
                    if ignoreError is False:
                        if (
                            ...
                            and response.json()["messsage"] is not None
                        ):
                ...
                errMsg = "GET Exception error {}/{} retries: {}".format(
                    restExecutionFailures, maxRetries, errMsg
                )
        ...
        while True:
            ...
            try:
                ...
                if silentMode is False:
                    ...
                    self.logInfo(
                        "\tSTATUS CODE: %s" % response.status_code, timestamp=False
                    )
                ...
                errMsg = "POST Exception error {}/{} retries: {}".format(
                    restExecutionFailures, maxRetries, errMsg
                )
    ...
    def patch(
        self, restApi, data={}, silentMode=False, ignoreError=False, maxRetries=5
    ):
        """
        Description
    ...
    def options(
        ...
        while True:
            ...
            try:
                ...
                errMsg = "OPTIONS Exception error {}/{} retries: {}".format(
                    restExecutionFailures, maxRetries, errMsg
                )
    ...
    def createWindowsSession(self, ixNetRestServerIp, ixNetRestServerPort="11009"):
        ...
        if ixNetRestServerIp is None:
            return

        url = "{0}://{1}:{2}/api/v1/sessions".format(
            self.httpScheme, ixNetRestServerIp, ixNetRestServerPort
        )
        ...
        if self.serverOs == "windowsConnectionMgr":
            ...
            for counter in range(1, counterStop + 1):
                ...
                self.logInfo(
                    "\n\tNew Windows session current state: {0}".format(currentState),
                    timestamp=False,
                )
        ...
        if self.serverOs == "windows":
            ...
            try:
                ...
                if "307" in str(response.status_code) and response.headers[
                    "location"
                ].startswith("https"):
    ...
    def getAllSessionId(self):
        ...
        for eachSessionId in response.json():
            sessionId.update(
                {
                    eachSessionId["id"]: {
                        "userName": eachSessionId["userName"],
            ...
            print(
                "   StartedOn: {}".format(eachSessionId["createdOn"].replace("T", " "))
            )
    ...
    def showErrorMessage(self, silentMode=False):
        ...
        errorList = []
        response = self.get(
            self.sessionUrl + "/globals/appErrors/error", silentMode=silentMode
        )
        print()
    ...
    def waitForComplete(
        ...
        Parameters
           ...
           ignoreException: (bool): ignoreException is for assignPorts.  Don't want to exit test.
                            Verify port connectionStatus for:
                                License Failed and Version Mismatch to report problem immediately.
        ...
        if silentMode is False:
            ...
            self.logInfo(
                "\tState:{}  Progress:{}".format(
                    response.json()["state"], response.json()["progress"]
                ),
                timestamp=False,
            )
        ...
        for counter in range(1, timeout + 1):
            ...
            if silentMode is False:
                if progress != 100:
                    self.logInfo(
                        "\tState:{}  Progress:{}:  Wait {}/{} seconds".format(
                            state, progress, counter, timeout
                        ),
                        timestamp=False,
                    )
                if progress == 100:
                    self.logInfo(
                        "\tState:{}  Progress:{}".format(state, progress),
                        timestamp=False,
                    )
    ...
    def waitForComplete_backup(
        ...
        Parameters
           ...
           silentMode: (bool):  If True, display info messages on stdout.
           ignoreException: (bool): ignoreException is for assignPorts.  Don't want to exit test.
                            Verify port connectionStatus for: License Failed and
                                            Version Mismatch to report problem immediately.
        ...
        for counter in range(1, timeout + 1):
            ...
            if silentMode is False:
                if state != "SUCCESS":
                    self.logInfo(
                        "\tState: {0}: Wait {1}/{2} seconds".format(
    ...
    def connectToLinuxApiServer(
        ...
        if self.apiKey is None:
            ...
            url = "https://{0}:{1}/api/v1/auth/session".format(
                linuxServerIp, linuxServerIpPort
            )
            self.logInfo("connectToLinuxApiServer: %s" % url)
            response = self.post(
                url, data={"username": username, "password": password}, ignoreError=True
            )
            if not str(response.status_code).startswith("2"):
                raise IxNetRestApiException("\nLogin username/password failed\n")
            self.apiKey = response.json()["apiKey"]

            url = "https://{0}:{1}/api/v1/sessions".format(
                linuxServerIp, linuxServerIpPort
            )
            if self.webQuickTest is False:
                data = {"applicationType": "ixnrest"}
            if self.webQuickTest is True:
                data = {"applicationType": "ixnetwork"}
            ...
            # https://10.10.10.1:443
            matchHeader = re.match(
                r"(https://[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+(:[0-9]+)?)", self.sessionId
            )
            self.httpHeader = matchHeader.group(1)
            ...
            if (
                self.linuxServerWaitForSuccess(response.json()["url"], timeout=timeout)
                == 1
            ):
                raise IxNetRestApiException

            if self.webQuickTest is True:
                self.sessionId = "https://{0}/ixnetworkweb/api/v1/sessions/{1}".format(
                    linuxServerIp, self.sessionIdNumber
                )
                self.sessionUrl = (
                    "https://{0}/ixnetworkweb/api/v1/sessions/{1}/ixnetwork".format(
                        linuxServerIp, self.sessionIdNumber
                    )
                )
                self.httpHeader = self.sessionUrl.split("/api")[0]
    ...
    def configLicenseServerDetails(
        self, licenseServer=None, licenseMode=None, licenseTier=None
    ):
        """
        Description
        ...
        self.patch(self.sessionUrl + "/globals/licensing", data=data)
    ...
    def linuxServerStopAndDeleteSession(self):
        ...
        """
        if self.serverOs == "linux" and self.deleteSessionAfterTest is True:
    ...
    def linuxServerWaitForSuccess(self, url, timeout=120):
        ...
        for counter in range(1, timeout + 1):
            ...
            self.logInfo(
                "\tCurrentStatus: {0}:  {1}/{2} seconds".format(
                    currentStatus, counter, timeout
                ),
                timestamp=False,
            )
            if (
                counter < timeout + 1
                and currentStatus != "Operation successfully completed"
            ):
                time.sleep(1)
    ...
    def refreshHardware(self, chassisObj):
        ...
        """
        response = self.post(
            self.sessionUrl + "/availableHardware/chassis/operations/refreshinfo",
            data={"arg1": [chassisObj]},
        )
        self.waitForComplete(
            response,
            self.sessionUrl + "/availableHardware/chassis/operations/refreshinfo",
    ...
    def query(self, data, silentMode=False):
        ...
        Examples
            ...
            bgpHostAttributes = response.json()['result'][0]['topology'][0]
                                ['deviceGroup'][0]['ethernet'][0]['ipv4'][0]['bgpIpv4Peer'][0]
            ...
            restObj.configMultivalue(bgpHostFlapMultivalue,
        ...
        self.waitForComplete(
            response, url + "/" + response.json()["id"], silentMode=silentMode
        )
        return response
    ...
    def configMultivalue(self, multivalueUrl, multivalueType, data):
        ...
        if multivalueType == "counter":
            # Examples: macAddress = {'start': '00:01:01:00:00:01',
            #                   'direction': 'increment', 'step': '00:00:00:00:00:01'}
            #          data=macAddress)
    ...
    def getMultivalueValues(self, multivalueObj, silentMode=False):
        ...
        """
        response = self.get(
            self.httpHeader + multivalueObj + "?includes=count", silentMode=silentMode
        )
        ...
        response = self.post(
            self.sessionUrl + "/multivalue/operations/getValues",
            data=data,
    ...
    @staticmethod
    def prettyprintAllOperations(self, sessionUrl):
        """
        Description
        ...
        if type(obj) == dict:
            ...
            for k, v in obj.items():
                if hasattr(v, "__iter__"):
                    print(
                        "%s%s:" % ((nested_level + 1) * spacing, k), file=output, end=""
                    )
                    # IxNetRestMain.printDict(v, nested_level + 1, output)
                    pass
                else:
                    print(
                        "%s%s: %s" % ((nested_level + 1) * spacing, k, v), file=output
                    )
