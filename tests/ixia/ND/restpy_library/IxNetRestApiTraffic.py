...
class Traffic(object):
    ...
    def configTrafficItem(self, mode=None, obj=None, trafficItem=None, endpoints=None, configElements=None):
        ...
        Parameters
            ...
            endpoints: [list]:
                A list: [{name: sources:[], destionations:[], highLevelStreams: None, (add more endpoints)... ]
        ...
        If mode is modify:
            The required parameters are: mode, obj, and one of the objects to modify
                                    (trafficIemObj, endpointObj or configElementObj).
        ...
        Traffic Item Parameters
            ...
            trackBy: [list]:
                trackingenabled0, ethernetIiSourceaddress0, ethernetIiDestinationaddress0, ethernetIiPfcQueue0,
        ...
        ConfigElement Parameters
            ...
            trackBy: <list>:
                Some options: flowGroup0, vlanVlanId0, ethernetIiDestinationaddress0, ethernetIiSourceaddress0,
        ...
        if mode == 'create' and trafficItem is not None:
            ...
            self.ixnObj.logInfo('configTrafficItem: %s : %s' % (trafficItemUrl, trafficItem),
                                timestamp=False)
            response = self.ixnObj.post(trafficItemUrl, data=trafficItem)
            trafficItemObj = response.json()['links'][0]['href']

        if mode == 'modify' and trafficItem is not None:
            trafficItemObj = obj
            if 'trackBy' in trafficItem:
                trackBy = trafficItem['trackBy']
                del trafficItem['trackBy']
            ...
            for endPoint in endpoints:
                ...
                if highLevelStream is not None:
                    ...
                    for eachHighLevelStream in highLevelStream:
                        self.configConfigElements(
                            self.ixnObj.httpHeader+trafficItemObj+'/highLevelStream/'+str(streamNum),
                            eachHighLevelStream
                        )
                        streamNum += 1

        if mode == 'modify' and endpoints is not None:
            endpointSrcDst = {}
        ...
        if isHighLevelStreamTrue is False and configElements is not None:
        ...
        # API server needs some time to complete processing highlevel stream configuration before entering regenerate.
        if mode == 'create' and trafficItem is not None:
            return [trafficItemObj, endpointSetObjList, configElementObjList]
    ...
    def getConfigElementObj(self, trafficItemObj=None, trafficItemName=None, endpointSetName=None):
        ...
        if trafficItemName:
            ...
            queryData = {
                ...
                [
                    {
                        'node': 'trafficItem', 'properties': ['name'],
                        'where': [{'property': 'name', 'regex': trafficItemName}]
                    },
                    {
                        'node': 'configElement',
                        'properties': ['endpointSetId'],
    ...
    def addTrafficItemPacketStack(self, configElementObj, protocolStackNameToAdd, stackNumber, action='append'):
        ...
        self.ixnObj.waitForComplete(
            response,
            self.ixnObj.httpHeader+configElementObj+'/stack/operations/appendprotocol/'+response.json()['id']
        )
    ...
    def getTrafficItemPktHeaderStackObj(self, configElementObj=None, trafficItemName=None, packetHeaderName=None):
        ...
        Description
           ...
           To get the displayName, you could call this function:
        ...
        for eachStack in response.json():
            ...
            currentStackDisplayName = eachStack['stackTypeId'].strip()
        ...
        raise IxNetRestApiException(
            '\nError: No such stack name found. Verify stack name existence and spelling: %s'
            % packetHeaderName)
    ...
    def getPacketHeaderStackIdObj(self, configElementObj, stackId):
        ...
        for (index, eachHeader) in enumerate(response.json()):
            ...
            if stackId == index+1:
                self.ixnObj.logInfo(
                    '\tReturning: %s' % self.ixnObj.httpHeader+eachHeader['links'][0]['href'],
                    timestamp=False
                )
    ...
    def modifyTrafficItemPacketHeader(self, configElementObj, packetHeaderName, fieldName, values):
        ...
        Description
           ...
           Use case #1: trafficItemName + endpointSetName
           Use case #2: trafficItemObj + endpointSetName
           Use case #3: trafficItemName only
                    (Will assume there is only one configElement object which will be returned)
           Use case #4: trafficItemObj only
                    (Will assume there is only one configElement object which will be returned)
    ...
    def modifyTrafficItemIpPriorityTos(self, trafficItemObj=None, trafficItemName=None, endpointSetName=None,
        ...
        Parameters
           value: <dict>: {
            'fieldValue': '000 Routine'|'001 Priority'|'010 Immediate'|'011 Flash'|'100 Flash Override'
    ...
    def modifyTrafficItemDestMacAddress(
            self,
            trafficItemObj=None,
    ...
    def modifyTrafficItemSrcMacAddress(
            self,
            trafficItemObj=None,
            trafficItemName=None,
            endpointSetName=None,
        ...
        self.modifyTrafficItemPacketHeader(configElementObj, packetHeaderName='ethernet',
                                           fieldName='Source MAC Address', values=values)
    ...
    def configPacketHeaderField(self, stackIdObj, fieldName, data):
        ...
        Example: To modify MPLS field:
            packetHeaderObj = trafficObj.getTrafficItemPktHeaderStackObj(
                trafficItemName='Raw MPLS/UDP', packetHeaderName='mpls')
    ...
    def getPacketHeaderAttributesAndValues(self, streamObj, packetHeaderName, fieldName):
        ...
        Parameters
            ...
            fieldName: <str>:
            Display Name of the field.  Example: If packetHeaderName is Ethernet II, field names could be
    ...
    def configEgressCustomTracking(self, trafficItemObj, offsetBits, widthBits):
        ...
        self.applyTraffic()
        self.ixnObj.patch(
            self.ixnObj.httpHeader+trafficItemObj+'/tracking/egress',
            data={
    ...
    def createEgressStatView(self, trafficItemObj, egressTrackingPort, offsetBit, bitWidth,
        ...
        response = self.ixnObj.post(
            self.ixnObj.sessionUrl+'/statistics/view',
            data={
                'caption': egressStatViewName,
        ...
        for eachTrackingFilter in response.json():
            self.ixnObj.logInfo('\tFilter Name: {0}: {1}'.format
                                (eachTrackingFilter['id'], eachTrackingFilter['name']),
                                timestamp=False)
            if bool(re.match(r'Custom: *\([0-9]+ bits at offset [0-9]+\)', eachTrackingFilter['name'])):
        ...
        if egressTrackingFilter is None:
            raise IxNetRestApiException('Failed to locate your defined custom offsets: {0}'.format
                                        (egressTrackingOffsetFilter))
        ...
        response = self.ixnObj.post(
            enumerationFilter,
            data={'sortDirection': 'ascending', 'trackingFilterId': egressTrackingFilter}
        )
        ...
        if ingressTrackingFilterName is not None:
            ...
            response = self.ixnObj.post(
                enumerationFilter,
                data={'sortDirection': 'ascending', 'trackingFilterId': ingressTrackingFilter}
            )
    ...
    def enableTrafficItem(self, trafficItemNumber):
        ...
        self.ixnObj.patch(url, data={"enabled": "true"})

    def disableTrafficItem(self, trafficItemNumber):
        url = self.ixnObj.sessionUrl+'/traffic/trafficItem/%s' % str(trafficItemNumber)
        self.ixnObj.patch(url, data={"enabled": "false"})
    ...
    def checkTrafficItemState(self, trafficItemList=None, expectedState=['stopped'], timeout=60, ignoreException=False):
        ...
        for trafficItemObj in trafficItemList:
            for counter in range(1, timeout+1):
                ...
                self.ixnObj.logInfo(
                    '\t{trafficState}: Expecting: {expectedStates}.'.format
                    (trafficState=currentTrafficState, expectedStates=expectedState),
                    timestamp=False
                )

                self.ixnObj.logInfo(
                    '\tWaited {counter}/{timeout} seconds'.format(counter=counter, timeout=timeout), timestamp=False)
                ...
                if counter == timeout and currentTrafficState not in expectedState:
                    if ignoreException is False:
                        raise IxNetRestApiException(
                            'checkTrafficState: ' +
                            'Traffic item state did not reach the expected state(s): {0}: {1}. It is at: {2}'.format
    ...
    def checkTrafficState(self, expectedState=['stopped'], timeout=60, ignoreException=False):
        ...
        for counter in range(1, timeout+1):
            ...
            self.ixnObj.logInfo(
                '\ncheckTrafficState: {trafficState}: Expecting: {expectedStates}.'.format
                (trafficState=currentTrafficState, expectedStates=expectedState),
                timestamp=False
            )
            self.ixnObj.logInfo(
                '\tWaited {counter}/{timeout} seconds'.format(counter=counter, timeout=timeout),
                timestamp=False
            )
        ...
        if ignoreException is False:
            raise IxNetRestApiException(
                'checkTrafficState: Traffic state did not reach the expected state(s): {0}. It is at: {1}'.format
                    ...
                    [
                        {
                            "node": "stack",
                            "properties": ["*"],
                            "where": [{"property": "stackTypeId", "regex": "ipv4"}]
                        },
                        {
                            "node":
                            "field",
    ...
    def getTrafficItemType(self, trafficItemName):
        ...
        if queryResponse.json()['result'][0]['trafficItem'] == []:
            raise IxNetRestApiException(
                'getTrafficItemType: No such Traffic Item Name found: %s'
                % trafficItemName
                )
    ...
    def getAllTrafficItemObjects(self, getEnabledTrafficItemsOnly=False):
        ...
        response = self.ixnObj.get(
            self.ixnObj.sessionUrl + '/traffic/trafficItem' +
            "?skip=" + str(numOfTrafficItem) + "&take=100"
            )

        while numOfTrafficItem < response.json()['count']:
            for eachTrafficItem in response.json()['data']:
                if getEnabledTrafficItemsOnly is True:
                    if eachTrafficItem['enabled'] is True:
    ...
    def applyTraffic(self):
        ...
        response = self.ixnObj.post(
            self.ixnObj.sessionUrl+'/traffic/operations/apply',
            data={'arg1': restApiHeader+'/traffic'}
            )
        self.ixnObj.waitForComplete(
            response,
            self.ixnObj.sessionUrl+'/traffic/operations/apply/'+response.json()['id']
            )
    ...
    def stopTraffic(self, blocking=False):
        ...
        if blocking is True:
            ...
            #             "node": "trafficItem",
            #             "properties": ["enabled"],
            #             "where": [{"property": "enabled", "regex": "True"}]
            #         }
            #     ]
            # }

            # queryResponse = self.ixnObj.query(data=queryData, silentMode=False)
            # enabledTrafficItemHrefList =
            #   [trafficItem['href'] for trafficItem in queryResponse.json()['result'][0]['trafficItem']]
        ...
        queryData = {
            ...
            [
                ...
                {'node': 'frameRate',     'properties': ['type', 'rate'], 'where': []},
                {
                    ...
                    'properties': ['streamDistribution', 'portDistribution'],
                    'where': []
                },
                {
                    'node': 'transmissionControl',
                    'properties': ['type', 'frameCount', 'burstPacketCount'],
                    'where': []
                },
