...
class FileMgmt(object):
    ...
    def loadConfigFile(self, configFile, localFile=True):
        ...
        if localFile is False:
            ...
            destinationPath = (
                "{0}/ixnetwork/files/".format(self.ixnObj.headlessSessionId) + fileName
            )
            response = self.ixnObj.post(
                copyFileUrl, data={"arg1": configFile, "arg2": destinationPath}
            )
            self.ixnObj.waitForComplete(
                response,
                self.ixnObj.httpHeader + "/" + response.json()["url"],
                silentMode=False,
                timeout=30,
            )

        if localFile is True:
            if os.path.exists(configFile) is False:
                raise IxNetRestApiException(
                    "Config file doesn't exists: %s" % configFile
                )

            if self.ixnObj.serverOs == "linux":
                octetStreamHeader = {
                    "content-type": "application/octet-stream",
                    "x-api-key": self.ixnObj.apiKey,
                }
            else:
            ...
            # Note:
            #    There is no status checking for this POST. This command is synchronous. T
            #                                           he server won't return until it's done.
            #    The response payload:
            # {'absolute': None, 'files': [{'name': 'bgp_ngpf_8.30.ixncfg', 'length': 177791,
            #               'modifiedUnixTime': 1572009307, 'createdUnixTime': 1568302806}], 'directories': []}
            response = self.ixnObj.post(
                uploadFile,
                data=configContents,
                noDataJsonDumps=True,
                headers=octetStreamHeader,
                silentMode=False,
            )
        ...
        payload = {
            "arg1": "{0}/ixnetwork/files/{1}".format(
                self.ixnObj.headlessSessionId, fileName
            )
        }
    ...
    def copyFileWindowsToRemoteWindows(
        self,
        ...
        Description
            ...
            The filename to be copied will remain the same filename unless
                 you set renameDestinationFile to something you otherwise preferred.
        ...
        # Step 1 of 2:
        self.ixnObj.post(
            self.ixnObj.sessionUrl + "/operations/copyfile",
            data={"arg1": windowsPathAndFileName, "arg2": destinationPath},
        )

        # curl http://{apiServerIp:port}/api/v1/sessions/1/ixnetwork/files/AggregateResults.csv
        #       -O -H "Content-Type: application/octet-stream" -output /home/hgee/AggregateResults.csv

        # Step 2 of 2:
        requestStatus = self.ixnObj.get(
            self.ixnObj.sessionUrl + "/files/%s" % (fileName),
            stream=True,
            ignoreError=True,
        )
        if requestStatus.status_code == 200:
            ...
            self.ixnObj.get(self.ixnObj.sessionUrl + "/files")
    ...
    def copyFileWindowsToLocalLinux(
        self,
        windowsPathAndFileName,
        localPath,
        renameDestinationFile=None,
        ...
        Description
            ...
            The filename to be copied will remain the same filename unless you set
                         renameDestinationFile to something you otherwise preferred.
            You could also include a timestamp for the destination file.
        ...
        self.ixnObj.post(
            url, data={"arg1": windowsPathAndFileName, "arg2": destinationPath}
        )

        # curl http://{apiServerIp:port}/api/v1/sessions/1/ixnetwork/files/AggregateResults.csv -O -H
        #           "Content-Type: application/octet-stream" -output /home/hgee/AggregateResults.csv
    ...
    def copyFileWindowsToLocalWindows(
        self,
        windowsPathAndFileName,
        localPath,
        renameDestinationFile=None,
        ...
        Example:
           WindowsPathAndFileName =
           'C:\\Users\\hgee\\AppData\\Local\\Ixia\\IxNetwork\\data\\result\\DP.Rfc2544Tput
                                \\9e1a1f04-fca5-42a8-b3f3-74e5d165e68c\\Run0001\\TestReport.pdf'
           localPath = 'C:\\Results'
        """
        import datetime
        ...
        self.ixnObj.logInfo(
            "\nYou need to manually remove the saved copy in: %s"
            % windowsPathAndFileName
        )
        fileName = windowsPathAndFileName.split("\\")[-1]
    ...
    def copyFileLinuxToLocalLinux(
        ...
        url = self.ixnObj.sessionUrl + "/operations/copyfile"
        response = self.ixnObj.post(
            url, data={"arg1": linuxApiServerPathAndFileName, "arg2": destinationPath}
        )
        ...
        response = self.ixnObj.get(
            self.ixnObj.sessionUrl + "/files?filename=%s" % (fileName),
            stream=True,
            ignoreError=True,
        )
    ...
    def importJsonConfigObj(
        self, dataObj, option="modify", silentMode=False, timeout=90
    ):
        """
        Description
            For newConfig:
        ...
        dataReformatted = {
            "arg1": "{0}/ixnetwork/resourceManager".format(
                self.ixnObj.headlessSessionId
            ),
    ...
    def importJsonConfigFile(self, jsonFileName, option="modify"):
        ...
        data = {
            ...
            "arg2": "{0}/ixnetwork/files/{1}".format(
                self.ixnObj.headlessSessionId, fileName
            ),
            "arg3": arg3,
        }
        ...
        self.ixnObj.waitForComplete(
            response, url + "/" + response.json()["id"], silentMode=False, timeout=300
        )
    ...
    def exportJsonConfigFile(self, jsonFileName, xpathList=None):
        ...
        data = {
            "arg1": sessionId + "/resourceManager",
            "arg2": xpathList,
            "arg3": True,
            "arg4": "json",
        ...
        response = self.ixnObj.post(url, data=data)
        self.ixnObj.waitForComplete(
            response, url + "/" + response.json()["id"], silentMode=False
        )
        ...
        if self.ixnObj.serverOs in ["windows", "windowsConnectionMgr"]:
            self.copyFileWindowsToLocalLinux(
                windowsPathAndFileName=absolutePath,
        ...
        if self.ixnObj.serverOs == "linux":
            self.copyFileLinuxToLocalLinux(
                linuxApiServerPathAndFileName=absolutePath,
                localPath=destinationPath,
                renameDestinationFile=None,
                includeTimestamp=False,
            )
    ...
    def exportJsonConfigToDict(self, xpathList=None):
        ...
        data = {
            "arg1": self.ixnObj.apiSessionId + "/resourceManager",
            "arg2": xpathList,
            "arg3": True,
            "arg4": "json",
        }
    ...
    def getJsonConfigPortList(self, jsonData):
        ...
        for vport in jsonData["vport"]:
            ...
            match = re.match(
                r"/availableHardware/.*'([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)']/card\[([0-9]+)]/port\[([0-9]+)",
                connectedTo,
            )
    ...
    def collectDiagnostics(self, diagZipFilename="ixiaDiagnostics.zip", localPath=None):
        ...
        response = self.ixnObj.post(url, data=data)
        self.ixnObj.waitForComplete(
            response, url + "/" + response.json()["id"], silentMode=False, timeout=900
        )
        response = self.ixnObj.get(self.ixnObj.sessionUrl + "/files")
        absolutePath = response.json()["absolute"]
        ...
        if self.ixnObj.serverOs in ["windows", "windowsConnectionMgr"]:
            absolutePath = absolutePath.replace("/", "\\")
            self.copyFileWindowsToLocalLinux(
                windowsPathAndFileName=absolutePath + "\\" + diagZipFilename,
                localPath=localPath,
            )

        if self.ixnObj.serverOs == "linux":
            self.copyFileLinuxToLocalLinux(
                linuxApiServerPathAndFileName=absolutePath + "/" + diagZipFilename,
                localPath=localPath,
            )
