...
class PortMgmt(object):
    ...
    #         self.ixnObj.logInfo(
    #             "connectToVChassis error: %s" % response.json()["result"]
    #         )
    #         return 1
    #     else:
    #         self.ixnObj.logInfo(
    #             "connectToVChassis: Successfully connected to chassis: %s" % chassisIp
    #         )
    #         while (
    ...
    def connectIxChassis(self, chassisIp, timeout=30, **kwargs):
        ...
        """
        if isinstance(chassisIp, list) is False:
        ...
        for chassisIpAddress in chassisIp:
            ...
            for timer in range(1, timeout + 1):
                response = self.ixnObj.get(
                    self.ixnObj.httpHeader + chassisIdObj, silentMode=True
                )
                currentStatus = response.json()["state"]
                self.ixnObj.logInfo(
                    "connectIxChassis {0}: Status: {1}. Wait {2}/{3} seconds".format(
                ...
                if currentStatus != "ready" and timer == timeout:
                    ...
                    # 'connectIxChassis: Connecting to chassis {0} failed'.format(chassisIpAddress))
                    ...
                    raise IxNetRestApiException(
                        "connectIxChassis failed. chassisIP:{0}.  It is in the {1} state. {2}".format(
                            chassisIp, currentStatus, errorMsg
                        )
                    )
    ...
    def getChassisId(self, chassisIp):
        ...
        """
        response = self.ixnObj.get(
            self.ixnObj.sessionUrl + "/availableHardware/chassis"
        )
    ...
    def connectVportTo(self, portList):
        ...
        for vportObj in vportObjectList.json():
            ...
            if connectedTo == "null":
                ...
                self.ixnObj.patch(
                    self.ixnObj.httpHeader + vportHref, data={"connectedTo": data}
                )
        ...
        for index in range(0, len(portList)):
            ...
            if portList is not None:
                ...
                response = self.ixnObj.patch(
                    self.ixnObj.httpHeader + vportObj, data={"name": portNumber}
                )
    ...
    def getVportObjectByName(self, portName):
        ...
        vportList = [
            "%s/vport/%s" % (self.ixnObj.sessionUrl, str(i["id"]))
    ...
    def linkUpDown(self, port, action="down"):
        ...
        self.ixnObj.post(
            self.ixnObj.sessionUrl + "/vport/operations/linkUpDn",
            data={"arg1": [vport], "arg2": action},
        )
    ...
    def assignPorts(
        ...
        if response.json() != []:
            ...
            vportList = [
                "/api%s/vport/%s" % (preamble, str(i["id"])) for i in response.json()
            ]
            if len(vportList) != len(portList):
                raise IxNetRestApiException(
                    "assignPorts: The amount of configured virtual ports:" +
                    "{0} is not equal to the amount of  portList:{1}".format(
                        len(vportList), len(portList)
                    )
                )

        else:
            if createVports is False:
                vportList = []
            ...
            if createVports:
                ...
                vportList = [
                    "/api%s/vport/%s" % (preamble, str(i["id"]))
                    for i in response.json()
                ]
                if len(vportList) != len(portList):
                    raise IxNetRestApiException(
        ...
        response = self.ixnObj.post(url, data=data)
        response = self.ixnObj.waitForComplete(
            response,
            ...
            for vport in vportResponse.json():
                ...
                for chassis, card, port in portList:
                    ...
                    if set(currentPort) & set(currentPortList):
                        ...
                        if vport["connectionStatus"] == "connectedLinkDown":
                            raise IxNetRestApiException(
                                "Port link connection is down: {0}".format(
                                    vport["assignedTo"]
                                )
                            )
        ...
        if response.json()["state"] == "IN_PROGRESS":
            raise IxNetRestApiException(
                "AssignPort failed: It has been 10 minutes and the ports " +
                "have not booted up successful. Something is wrong. Maybe you need to reboot the port CPU."
            )
        ...
        if configPortName:
            ...
            for vportObj in self.getAllVportList():
                ...
                port = port.split(":")[2]
                self.ixnObj.patch(
                    self.ixnObj.httpHeader + vportObj,
                    data={"name": "Port" + card + "_" + port},
                )
    ...
    def unassignPorts(self, deleteVirtualPorts=False):
        ...
        response = self.ixnObj.post(
            url, data={"arg1": vportList, "arg2": deleteVirtualPorts}
        )
        self.ixnObj.waitForComplete(
            response,
            self.ixnObj.sessionUrl
            + "/vport/operations/unassignports/"
            + response.json()["id"],
            timeout=120,
        )
    ...
    def releaseAllPorts(self):
        ...
        if response.json()["id"] != "":
            self.ixnObj.waitForComplete(
                response, url + "/" + response.json()["id"], timeout=120
            )
    ...
    def arePortsAvailable(self, portList, raiseException=True):
        ...
        for port in portList:
            ...
            try:
                    ...
                    "nodes": [
                        {
                            "node": "chassis",
                            "properties": ["ip"],
                            "where": [{"property": "ip", "regex": chassisIp}],
                        },
                        {
                            "node": "card",
                            "properties": ["cardId"],
                            "where": [{"property": "cardId", "regex": cardId}],
                        },
                        {
                            "node": "port",
                            "properties": ["portId", "owner"],
                            "where": [{"property": "portId", "regex": portId}],
                        },
                    ],
                }

                self.ixnObj.logInfo(
                    "Querying for %s/%s/%s" % (chassisIp, cardId, portId)
                )
                ...
                queryResponse.json()["result"][0]["chassis"][0]["card"][0]["port"][0][
                    "portId"
                ]
            except Exception:
                raise IxNetRestApiException(
            ...
            if (
                ...
                self.ixnObj.logInfo(
                    "Port is still owned: {0}/cardId:{1}/portId:{2}".format(
                        chassisIp, cardId, portId
                    )
                )
    ...
    def verifyPortState(self, timeout=70):
        ...
        for eachVport in vportList:
            for counter in range(1, timeout + 1):
                stateResponse = self.ixnObj.get(
                    self.ixnObj.httpHeader
                    + eachVport
                    + "?includes=state,connectionStatus",
                    silentMode=True,
                )
                assignedToResponse = self.ixnObj.get(
                    self.ixnObj.httpHeader + eachVport + "?includes=assignedTo",
                    silentMode=True,
                )

                if "Port Released" in stateResponse.json()["connectionStatus"]:
                    raise IxNetRestApiException(
                        stateResponse.json()["connectionStatus"]
                    )

                if stateResponse.json()["state"] == "unassigned":
                    self.ixnObj.logWarning(
                        "\nThe vport {0} is not assigned to a physical port. Skipping this vport verification.".format(
                            eachVport
                        )
                    )
                    break
                ...
                self.ixnObj.logInfo(
                    "Port: %s" % assignedToResponse.json()["assignedTo"]
                )
                self.ixnObj.logInfo(
                    "\tVerifyPortState: %s\n\tWaiting %s/%s seconds"
                    % (stateResponse.json()["state"], counter, timeout),
                    timestamp=False,
                )
                if counter < timeout and stateResponse.json()["state"] in [
                    "down",
                    "busy",
                ]:
                    time.sleep(1)
    ...
    def getVportFromPortList(self, portList):
        ...
        for eachPort in portList:
            ...
            if vport == []:
                raise IxNetRestApiException(
                    "getVportFromPortList error: The port has no vport and not assigned. " +
                    "Check for port typo: {0}".format(
                        port
                    )
                )
    ...
    def modifyPortMediaType(self, portList="all", mediaType="fiber"):
        ...
        for vport in vportList:
            ...
            self.ixnObj.patch(
                self.ixnObj.httpHeader + vport + "/l1Config/" + portType,
    ...
    def modifyL1Config(self, configSettings, portList="all"):
        ...
        """
        self.ixnObj.logInfo(
            "modifyL1Config: portList = {} configSettings = {}".format(
                portList, configSettings
            )
        )
        response = self.ixnObj.get(self.ixnObj.sessionUrl + "/vport")
        if portList == "all":
    ...
    def configLoopbackPort(self, portList="all", enabled=True):
        ...
        for vport in vportList:
            response = self.ixnObj.get(
                self.ixnObj.httpHeader + vport + "/l1Config", silentMode=False
            )
            currentPortType = response.json()["currentType"]
            self.ixnObj.patch(
                self.ixnObj.httpHeader + vport + "/l1Config/" + currentPortType,
    ...
    def setTxMode(self, vportList="all", txMode="interleaved", timeout=70):
        ...
        for eachVport in vportList:
            self.ixnObj.patch(
                self.ixnObj.httpHeader + eachVport, data={"txMode": txMode}
            )

    def configUdsRxFilters(
        self, portList="all", filterPalette=None, udsNum="1", udsArgs=None
    ):
        """
        Description
        ...
        """
        self.ixnObj.logInfo(
            "configUdsRxFilters: filterPalette={0}".format(filterPalette)
        )
        self.ixnObj.logInfo("\t\t uds={0} udsArgs={1}".format(udsNum, udsArgs))
        self.ixnObj.get(self.ixnObj.sessionUrl + "/vport")
        if portList == "all":
            # vportList = self.getAllVportList()
            portList = self.getPhysicalPortsFromCreatedVports()
        ...
        vportList = self.getVportFromPortList(portList)
        for vport in vportList:
            self.ixnObj.get(self.ixnObj.httpHeader + vport, silentMode=True)
            self.ixnObj.patch(
                self.ixnObj.httpHeader + vport + "/l1Config/rxFilters/filterPalette",
                data=filterPalette,
            )
            self.ixnObj.patch(
                self.ixnObj.httpHeader + vport + "/l1Config/rxFilters/uds/" + udsNum,
                data=udsArgs,
            )
